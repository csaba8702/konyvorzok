...
//use Config\Aauth as Authconfig;
 ...
 * Modified to Codeigniter 4 PHP framework.
 */
class Aauth {
	...
	public function __construct() {
		...
		$this->config_vars = config('Aauth')->config_aauth['default'];


		// load error and info messages from flashdata (but don't store back in flashdata)
		$this->errors = $this->CI->session->setflashdata('errors') ?: array();
		$this->infos = $this->CI->session->setflashdata('infos') ?: array();
	}
	 ...
	 */
	public function create_user($email, $pass, $username = FALSE, $lastname = '', $firstname = '') {
		...
		*/
		...
		$data = array(
			'email' => $email,
			'password' => $this->hash_password($pass, 0), // Password cannot be blank but user_id required for salt, setting bad password for now
		...
		if ( $this->aauth_db->insert($this->config_vars['users'], $data )){
			...
			// users_extra_data
			$this->CI->load->config('site_db');
			$extra_data = array('user_id' => $user_id);
			$this->aauth_db->insert($this->CI->config->item('users_extra_data_table'),$extra_data);
	...
	public function send_verification($user_id){
		...
		if ($query->num_rows() > 0){
			...
			$this->CI->email->message($this->CI->lang->line('aauth_email_verification_code') . "<b>" . $ver_code . "</b><br/>" .
				$this->CI->lang->line('aauth_email_verification_text') . "<br/>" . site_url() .$this->config_vars['verification_link'] . $user_id."/".$ver_code );
			$this->CI->email->send();
		}
	}
